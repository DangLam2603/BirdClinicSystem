...
<%@page import="DAO.StaffsDAO"%>
<%@page import="DAO.CustomersDAO"%>
<%@page import="DTO.Staffs"%>
            ...
            <ul class="nav-links">
                ...
                <span class="dropdown-container">
                    ...
                    <div class="fadeIn second" style="padding: 10px; background: #95EBD1;">
                        <a class="hover_div_cus" href="MainController?action=StaffList"><i class="fa-solid fa-user-gear" style="margin: 5px; margin-right: 13px; color: black"></i>Staffs</a>
                    </div>
                ...
                <div class="sales-boxes">
                    <div class="recent-sales box">
                        <form action="MainController" method="POST">
                            <div class="row1">
                                <c:set var="totalStaff" value="0"></c:set>
                                <c:forEach var="list" items="${StaffsDAO.getAllStaff()}">
                                    <c:set var="totalStaff" value="${totalStaff + 1}" />
                                </c:forEach>
                                <div class="col-sm-2">
                                    <h4 style="margin:0px;">Staffs</h4>
                                    <p style="margin:0px; color:rgba(201, 202, 202, 1);">Show ${totalStaff} Staffs</p>
                                </div>
                                <div class="col-sm-2" style="border-bottom: 1px solid">
                                    ...
                                    <button value="StaffList" style="border:none; background-color: white;" name="action">
                                ...
                                <select name="txtsearchby" class="selecttextbox col-sm-2" style="width: 20%; margin-left: 25px">
                                    <option value="staffid" ${requestScope.searchby == 'staffid' ? 'selected' : ''}>ID</option>
                                    <option value="staffmail" ${requestScope.searchby == 'staffmail' ? 'selected' : ''}>Email</option>
                                    <option value="staffname" ${requestScope.searchby == 'staffname' ? 'selected' : ''}>Name</option>
                                    <option value="staffphone" ${requestScope.searchby == 'staffphone' ? 'selected' : ''}>Phone</option>
                                </select>
                                ...
                                <div class="col-sm-2">
                                    <a class="button" style="padding: 20px 30px;" href="ADB_AddStaff.jsp">Add New Staff</a>
                                </div>
                        ...
                        <table border="1" width="100%" >                                
                            <tbody>
                                ...
                                <c:forEach var="staff" items="${requestScope.staffList}" varStatus="status">
                                    <tr>
                                        ...
                                        <td style="text-align: center;">
                                            <c:url var="mylink" value="MainController">
                                                <c:param name="email" value="${staff.getEmail()}"></c:param>
                                                <c:param name="status" value="${staff.getStatus()}"></c:param>
                                                <c:param name="action" value="UpdateStatus"></c:param>
                                                <c:param name="source" value="staffList"></c:param>
                                            </c:url>
                                            <c:if test="${staff.getStatus() eq 0}">
                                                <a class="button" href="${mylink}">Active</a>
                                            </c:if>
                                            <c:if test="${staff.getStatus() eq 1}">
